...
class CKBCProcessor(DataProcessor):
    """Processor for CKBC data set."""

    def __init__(self):
        self.labels = set()
        self.use_sentence_template = False

        if self.use_sentence_template:
            raw_rel2text = json.load(open("relation_map.json"))
        else:
            raw_rel2text = {
        ...
        self.rel2text = {k.lower(): v for k, v in raw_rel2text.items()}

    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train100k.txt")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "dev1.txt")), "dev")
    ...
    def _create_examples(self, lines, set_type):
        """Creates examples for the training and dev sets."""
        # rel2text = {r: " ".join(camel_case_split(r)).lower()
        #            for r in set([line[0] for line in lines])}
        ...
        for (i, line) in tqdm(enumerate(lines)):
            ...
            relation_text = self.rel2text[relation.lower()]
            ...
            if set_type == "dev" or set_type == "test":
                if len(line) >= 4:
                    ...
                    label = "1"     # if no labels, use 1 as default
                ...
                examples.append(InputExample(
                    ...
                    text_c=text_c, relation=relation, label=label))
                ...
                examples.append(InputExample(
                    guid=guid, text_a=text_a, text_b=text_b,
                    text_c=text_c, relation=relation, label="1"))
                ...
                if rnd <= 0.5:
                    ...
                    while True:
                        ...
                        if (tmp_head, relation, tail) not in triples:
                            tmp_head_text = tmp_head
                            break

                    assert tmp_head_text != ""
                    examples.append(InputExample(
                        guid=guid, text_a=tmp_head_text, text_b=text_b,
                        text_c=text_c, relation=relation, label="0"))
                else:
                    # corrupting tail
                    tmp_tail_text = ""
                    while True:
                        ...
                        if (head, relation, tmp_tail) not in triples:
                            tmp_tail_text = tmp_tail
                            break

                    assert tmp_tail_text != ""
                    examples.append(InputExample(
                        guid=guid, text_a=text_a, text_b=text_b,
                        text_c=tmp_tail_text, relation=relation, label="0"))
        return examples
...
def eval_on_dev_set(args, model, tokenizer, processor, device, task_name,
                    label_list, num_labels, global_step, tr_loss, nb_tr_steps, epoch=-1):
    ...
    if epoch != -1:
        output_eval_file = os.path.join(args.output_dir, "eval_results_epoch_{}.txt".format(epoch))
    else:
...
def main():
    ...
    parser.add_argument("--save_best_model",
                        action='store_true',
                        help="Whether to save best model on dev set.")
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
                if args.save_best_model:
                    dev_score = eval_on_dev_set(
                        args, model, tokenizer, processor, device, task_name,
                        label_list, num_labels, global_step, tr_loss, nb_tr_steps, epoch=epoch)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model.to(device)
        model.eval()
        eval_on_dev_set(args, model, tokenizer, processor, device, task_name,
                        label_list, num_labels, global_step, tr_loss, nb_tr_steps)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        test_files = [f for f in os.listdir(args.test_data_dir)
                      if os.path.isfile(os.path.join(args.test_data_dir, f))
                      and os.path.splitext(os.path.basename(f))[0] in processor.get_relation_set()]

        for test_file in test_files:
            logger.info("Evaluating file {} ...".format(test_file))
            eval_examples = processor.get_test_examples(
                os.path.join(args.test_data_dir, test_file))
            ...
            preds = preds[0]
            scores = list(torch.nn.functional.softmax(torch.tensor(preds), dim=1).numpy()[:, 1])
            scored_samples = []
            for example, score in zip(eval_examples, scores):
                scored_samples.append((example.relation, example.text_a, example.text_c, score))
            ...
            output_file = os.path.join(args.prediction_dir, test_file)
